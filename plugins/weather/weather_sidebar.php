...
function weather_sidebar($values) {
	...
	$output = "<script>'use strict';geoLocation('".COOKIE_PATH."');</script>";
	if($now['cod'] != 200) {
		$output .= '
		<div class="sidebar-container widget-weather-plugin">
			<div class="sidebar-content">
				...
				<div class="sidebar-inner overflow-auto">'.$LNG['plugin_weather_error'].'</div>
			</div>
		</div>';
	} else {
	    $forecast = '';
